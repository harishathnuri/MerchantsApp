// <auto-generated />
using System;
using Merchants.Web.DbContexts;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

namespace Merchants.Web.Migrations
{
    [DbContext(typeof(MerchantContext))]
    [Migration("20200923065424_InitialMigration")]
    partial class InitialMigration
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .UseIdentityColumns()
                .HasAnnotation("Relational:MaxIdentifierLength", 128)
                .HasAnnotation("ProductVersion", "5.0.0-rc.1.20451.13");

            modelBuilder.Entity("Merchants.Web.Entities.Country", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Name")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("Id");

                    b.ToTable("Countries");

                    b.HasData(
                        new
                        {
                            Id = new Guid("eef7fb91-f2a4-42d2-9c7d-e30ab7cdfd44"),
                            Name = "Australia"
                        },
                        new
                        {
                            Id = new Guid("2288f577-e965-4727-bd28-7a5cf2247f5d"),
                            Name = "New Zealand"
                        },
                        new
                        {
                            Id = new Guid("a81f1301-903b-45ca-840c-533077523af5"),
                            Name = "India"
                        },
                        new
                        {
                            Id = new Guid("4c0907f9-2c5f-4d3e-bc5d-ce5c49dc66b8"),
                            Name = "USA"
                        },
                        new
                        {
                            Id = new Guid("def5f724-7794-4299-810c-0c5b2d89c7dd"),
                            Name = "Great Britain"
                        });
                });

            modelBuilder.Entity("Merchants.Web.Entities.Currency", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Name")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("Id");

                    b.ToTable("Currencies");

                    b.HasData(
                        new
                        {
                            Id = new Guid("038e3b65-ffdf-4f23-8767-ee2f079eb160"),
                            Name = "AUD"
                        },
                        new
                        {
                            Id = new Guid("aac9e06c-aa9d-4e6a-9a1a-ef7189daf38f"),
                            Name = "NZD"
                        },
                        new
                        {
                            Id = new Guid("590e1142-81f9-4691-a379-c6ff97f21bbc"),
                            Name = "INR"
                        },
                        new
                        {
                            Id = new Guid("a3fced19-470b-43f3-913f-b77e1abc3ae7"),
                            Name = "USD"
                        },
                        new
                        {
                            Id = new Guid("d1577738-f7e4-4565-9f1d-901daf6669b6"),
                            Name = "GBP"
                        });
                });

            modelBuilder.Entity("Merchants.Web.Entities.Merchant", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CountryId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CurrencyId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<double>("DiscountPercentage")
                        .HasColumnType("float");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.Property<string>("WebsiteUrl")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("CountryId");

                    b.HasIndex("CurrencyId");

                    b.ToTable("Merchants");
                });

            modelBuilder.Entity("Merchants.Web.Entities.Merchant", b =>
                {
                    b.HasOne("Merchants.Web.Entities.Country", "Country")
                        .WithMany()
                        .HasForeignKey("CountryId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Merchants.Web.Entities.Currency", "Currency")
                        .WithMany()
                        .HasForeignKey("CurrencyId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Country");

                    b.Navigation("Currency");
                });
#pragma warning restore 612, 618
        }
    }
}
